...
export const Planner = () => {
    ...
    const handleOutfitSelect = async (lb: any, date: string) => {
        ...
        // Get current dates and add the new one
        const currentPlannedDates = Array.isArray(lb.plannedUsed) ? lb.plannedUsed : [];
        
        // Check if date already exists to avoid duplicates on the same day
        if (currentPlannedDates.includes(date)) {
        ...
        try {
            ...
            const res = await fetch(buildPath(`api/lists/${lookbookId}`), {
                ...
                body: JSON.stringify({ 
                    // Spread existing data and update the array
                    plannedUsed: [...currentPlannedDates, date], 
    ...
    const handleConfirmRemove = async () => {
        ...
        const dateToRemove = lookbookToRemove.currentEventDate; // This was injected during flatMap
        const storedUser = sessionStorage.getItem("user_data");
        const token = storedUser ? JSON.parse(storedUser).token : "";

        try {
            
            // Create the new array by filtering out the date we are removing
            const newPlannedDates = lookbookToRemove.plannedUsed.filter(
            ...
            const res = await fetch(buildPath(`api/lists/${lookbookId}`), {
                ...
                body: JSON.stringify({ 
                    plannedUsed: newPlannedDates // Send the filtered array
                }),
            });
    ...
    const events = lookbooks.flatMap(lb => {
        
        //Ensure plannedUsed is an array.
        const dates: string[] = Array.isArray(lb.plannedUsed) ? lb.plannedUsed : [];
        
        return dates
            .filter(date => {
                // Filter out the default "empty" dates
                return date && !date.startsWith('1970-01-01');
            })
            .map(date => ({
                // Create a unique ID combining ID and Date so FullCalendar can distinguish them
                id: `${lb._id || lb.id}-${date}`, 
                title: lb.title,
                start: date,
                allDay: true,
                // inject the specific date into extendedProps 
                // so we know WHICH instance was clicked later.
                extendedProps: { ...lb, currentEventDate: date } 
            }));
    });
        ...
        return (
            ...
            <button 
                ...
                className={`flex items-center justify-center w-full p-1 rounded-md transition-all duration-300 cursor-pointer relative z-10 hover:z-50 hover:scale-105 origin-center shadow-sm border backdrop-blur-md ${
                    isUsed 
                        ? "bg-green-400/50! text-white border-green-400/50 hover:brightness-125" 
                        : "bg-secondary! text-black border-secondary/30 hover:brightness-110"
                }`}
            >
                ...
                <div className="flex items-center justify-center gap-1.5 truncate pointer-events-none w-full px-2">
                    ...
                    <span className="truncate text-[8px] sm:text-[9px] uppercase tracking-[0.12em] font-bold py-0.5">
